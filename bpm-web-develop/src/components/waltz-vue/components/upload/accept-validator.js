...
export class AcceptValidator {
  static parse(accept) {
    ...
    const nameParts = parts.filter(p => p.startsWith(".")).map(p => matchAll + escapeForPattern(p));
    const typeParts = parts
      .filter(p => !p.startsWith("."))
      .map(part => {
        const [type, subType] = part.split("/", 2);
        return subType === "*" ? escapeForPattern(`${type}/`) + matchAll : escapeForPattern(part);
      });
  ...
  constructor(namePattern = null, typePattern = null) {
  ...
  isValid(file) {
    ...
    return (
      (this.namePattern && this.namePattern.test(file.name)) || (this.typePattern && this.typePattern.test(file.type))
    );
  }
}
